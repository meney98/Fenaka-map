...
function clamp(){
  ...
  const pad=30;
  if(sw<=vw){ x=(vw-sw)/2; }
  else { x=Math.min(pad,Math.max(vw-sw-pad,x)); }
  if(sh<=vh){ y=(vh-sh)/2; }
  else { y=Math.min(pad,Math.max(vh-sh-pad,y)); }
}
...
img.addEventListener("load",()=>requestAnimationFrame(()=>requestAnimationFrame(fit)));
